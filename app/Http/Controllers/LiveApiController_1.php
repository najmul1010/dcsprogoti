...
class LiveApiController extends Controller
{
  ...
  public function DcsDataSync(Request $request)
  {
    ...
    if ($ProjectCode == '060') {
      $otherapi = (new LiveApiProgotiController)->ProgotiSync($db, $apikey, $key, $token, $BranchCode, $ProjectCode, $LastSyncTime, $Appid, $Pin, $AppVersionCode, $AppversionName, $CurrentTimes);
      die;
    ...
    try {
      ...
      //dd("Test");
      $memberlistdata = $this->MemberLists($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid, $key);
      ...
      //dd("huda");
      $loandata = $this->LoanDataSync($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid);
      //dd($loandata);
  ...
  {
    $this->Admission_GetDataForErpStatusCheck($db, $db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid);
    $ErpStatusId = null;
  ...
  public function LoanDataSync($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid)
  {
    $this->Loan_GetDataForErpStatusCheck($db, $db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid);
    if ($Appid == 'bmsmerp') {
      $dataset = array();
      $loandatas = array();
      $polist = $this->PoList($db, $BranchCode, $ProjectCode, $Pin);
  ...
  public function MemberLists($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid, $key)
  {
    //dd("Huda");
    ...
    if ($Appid == 'bmsmerp') {
      //dd("t");
      $polist = DB::Table($db . '.polist')->where('branchcode', $BranchCode)->where('projectcode', $ProjectCode)->where('cono', '!=', $Pin)->where('status', 1)->get();
      //dd($polist);
      if (!$polist->isEmpty()) {
  ...
  public function Admission_GetDataForErpStatusCheck($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid)
  {
    if ($Appid == 'bmsmerp') {
  ...
  public function Loan_GetDataForErpStatusCheck($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid)
  {
    if ($Appid == 'bmsmerp') {
  ...
  public function NID_Verification(Request $req)
  {
    ...
    curl_close($curl);
    Log::info("Nid Verification OUT" . $output_colsed);
