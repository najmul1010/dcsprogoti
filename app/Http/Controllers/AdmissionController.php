...
class AdmissionController extends Controller
{
    ...
    public function index()
    {
        ...
        if (session('projectcode') == '060') {
            return view('progoti/admission-request')->with('branch', $branch)->with('value', $value)->with('search2', $search2)->with('status', $status);
        } else {
    ...
    public function admissionTable(Request $req)
    {
        ...
        if ($division != null && $region == null && $area == null && $branch_search == null && $po_search == null && $status_search == null && $po_search == null) {
            ...
            return datatables($datas)->addColumn('branchcode', function ($datas) {
                ...
                $time = date('Y-m-d', strtotime($datas->created_at));
                return $time;
    ...
    public function branch(Request $request)
    {
        ...
        $projectcode = session('projectcode');
        /* $url = "http://scm.brac.net/sc/GetPO_Data?branchcode=$branchCode&projectcode=$projectcode";
        $ch = curl_init();
    ...
    public function admission_approve($id)
    {
        ...
        if (session('projectcode') == '060') {
            return view('progoti/admissionApproval')->with('data', $data)->with('data2', $data2);
        } else {
    ...
    public function approve_admission(Request $request)
    {
        ...
        $document_url = "http://34.126.65.194/dcs/DocumentManager?doc_id=$doc_id&projectcode=$projectcode&doc_type=admission&pin=$pin&role=$role&branchcode=$branchcode&action=$action";
        // dd($document_url);
        ...
        if ($collectionfordocument != NULL) {
            if ($collectionfordocument->status == "E") {
                if (isset($collectionfordocument->errors)) {
                    $filderrorcheck  = $collectionfordocument->errors[0];
                    $getMessage = $filderrorcheck->fieldErrors;
                    $errors = $getMessage[0]->message;
                    // dd($errors);
                ...
                return redirect('/operation/admission');
    ...
    public function action_btn(Request $request)
    {
        ...
        $document_url = "http://34.126.65.194/dcs/DocumentManager?doc_id=$doc_id&projectcode=$projectcode&doc_type=admission&pin=$pin&role=$role&branchcode=$branchcode&action=$action&comment=$comment";
        // dd($document_url);
        Log::channel('daily')->info('Document_url : ' . $document_url);
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $document_url);
