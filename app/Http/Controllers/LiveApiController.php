...
class LiveApiController extends Controller
{
  ...
  public function DcsDataSync(Request $request)
  {
    ...
    $LastSyncTime = date('Y-m-d H:i:s', strtotime('-3 days', strtotime($LastSyncTime)));
    ...
    if ($ProjectCode == '060') {
      $otherapi = (new LiveApiProgotiController)->ProgotiSync($db, $dberp, $apikey, $key, $token, $BranchCode, $ProjectCode, $LastSyncTime, $Appid, $Pin, $AppVersionCode, $AppversionName, $CurrentTimes);
      die;
    } else if ($ProjectCode == '015') {
    } else {
      $this->CUSTMSG("Project Code Not Found!");
    }
    try {
      // dd("Huda");
  ...
  public function AdmissionDataSync($db, $dberp, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid)
  {
    ...
    if ($Appid == 'bmsmerp') {
      ...
      $checkDesig = $this->Individual_GetPO($BranchCode, $ProjectCode, $Pin);
      if ($checkDesig[0]->desig == 'Assistant Branch Manager') {
        $polist = $this->GET_ABM_POList($BranchCode, $ProjectCode);
      } else {
        $polist = $this->GET_POList($BranchCode, $ProjectCode);
      }
      foreach ($polist as $row) {
        ...
        if (!empty($admissiondata)) {
          foreach ($admissiondata as $data) {
            ...
            $checkPolistForABM = DB::Table($db . '.polist')->where('cono', $cono)->where('abm', '!=', '')->where('status', 1)->get();
            //dd($checkPolistForABM);
            if ($checkPolistForABM->isEmpty()) {
              $status = $status->status_name ?? null;
            } else {

              $status = "ABM " . $status->status_name;
            }
            if ($data->IsBkash == '1') {
              $IsBkash = "Yes";
            } else {
            ...
            $arrayData = array(
              ...
              'houseImagePath' => $data->bm_client_house_image ?? null,
              "SpousePlaceofIssue" =>$data->SpouseIdPlaceOfissue ?? null,
              "SpouseIdExpairedate" => $data->SpouseIdExpiredate ?? null,
              "SpouseIdIssuedate" => $data->spouseidissuedate ?? null
            );
            //Log::info("PIN" . $cono);
      ...
      if (!empty($admissiondata)) {
        foreach ($admissiondata as $data) {
          ...
          $PermanentDistrictId = $PermanentDistrictId->district_name ?? null;
          $comment = $dochistory->comment ?? null;
          $checkPolistForABM = DB::Table($db . '.polist')->where('cono', $Pin)->where('abm', '!=', '')->where('status', 1)->get();
          if ($checkPolistForABM->isEmpty()) {
            $status = $status->status_name ?? null;
          } else {
        ...
        if (!empty($loandata)) {
          foreach ($loandata as $data) {
            ...
            if ($checkPolistForABM->isEmpty()) {
              $status =$status->status_name ?? null;
              /*if ($checkDesig[0]->desig == 'Assistant Branch Manager') {
                if ($data->roleid == 1 and $data->reciverrole == 1 and $data->status = '3') {
                  $status = "ABM " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 1 and $data->status = '2' and $data->erpstatus = 1) {
                  $status = "ERP " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 1 and $data->status = '2' and $data->erpstatus = 2) {
                  $status = "ERP " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 1 and $data->status = '2' and $data->erpstatus = 3) {
                  $status = "ERP " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 1 and $data->status = '2' and $data->erpstatus = 4) {
                  $status = "ERP " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 2) {
                  $status = "AM " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 3) {
                  $status = "RM " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 4) {
                  $status = "DM " . $status->status_name;
                } else if ($data->roleid == 2 and $data->reciverrole == 1) {
                  $status = "ABM " . $status->status_name;
                } else if ($data->roleid ==  and $data->reciverrole == 1) {
                  $status = "ABM " . $status->status_name;
                }
                else if ($data->roleid == 0 and $data->reciverrole == 1) {
                  $status = "ABM " . $status->status_name;
                } else if ($data->roleid == 1 and $data->reciverrole == 0) {
                  $status = "ABM Sendback";
                } else if ($data->roleid == 1 and $data->reciverrole == 1) {
                  $status = "ABM " . $status->status_name;
                }
              }
              else
              ...
              $status = "ABM " . $status->status_name;
              /*if ($data->roleid == 0 and $data->reciverrole == 1 and $data->status = '3') {
                $status = "ABM " . $status->status_name;
              } else if ($data->roleid == 1 and $data->reciverrole == 1 and $data->status = '2' and $data->erpstatus = 1) {
                $status = "ERP " . $status->status_name;
              } else if ($data->roleid == 1 and $data->reciverrole == 1 and $data->status = '2' and $data->erpstatus = 2) {
            ...
            $arrayData['loan'] = array(
              ...
              "bracloan_family" => $brackloanfamily,
  ...
  public function MemberLists($db, $dberp, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid, $key)
  {
    ...
    if ($Appid == 'bmsmerp') {
      ...
      //dd($polist);
      //$this->GET_POList($branchCode, $projectcode);
      $checkDesig = $this->Individual_GetPO($BranchCode, $ProjectCode, $Pin);
      //dd($checkDesig);
      if ($checkDesig[0]->desig == 'Assistant Branch Manager') {
        $polist = $this->GET_ABM_POList($BranchCode, $ProjectCode);
  ...
  public function NID_Verification(Request $req)
  {
    ...
    if ($versionCode != '') {
      if ($jsondecodeNID != null) {
        $suspectMemberjsondecode = $jsondecodeNID->suspectedMember;
        if (!empty($suspectMemberjsondecode)) {
          $branchcode = $suspectMemberjsondecode[0]->branchCode;
          $po = $suspectMemberjsondecode[0]->assignedPoPin;
          if (isset($suspectMemberjsondecode[0]->voCode)) {
            $voCode = $suspectMemberjsondecode[0]->voCode;
          } else {
            $voCode = '';
          }

          $memberName = $suspectMemberjsondecode[0]->memberName;
          $memberNo = $suspectMemberjsondecode[0]->memberNo;
          $getPoInfo = DB::Table($db . '.polist')->where('cono', $po)->where('branchcode', $branchcode)->get();
          //dd($getPoInfo);
          if (!$getPoInfo->isEmpty()) {
            $poname = $getPoInfo[0]->coname;
          } else {
            $poname = '';
          }
          $getBranchInfo = DB::Table('branch')->where('branch_id', $branchcode)->where('program_id', 1)->get();
          if (!$getBranchInfo->isEmpty()) {
            $branchname = $getBranchInfo[0]->branch_name;
            $areaname = $getBranchInfo[0]->area_name;
            $regionname = $getBranchInfo[0]->region_name;
            $divisionname = $getBranchInfo[0]->division_name;
          } else {
            $branchname = '';
            $areaname = '';
            $regionname = '';
          ...
          $getPOPhone = DB::Table($db . '.contacts_info')->where('branchcode', (int)$branchcode)->where('project', 'Dabi')->get();
          if (!$getPOPhone->isEmpty()) {
            $phoneno = $getPOPhone[0]->official_mobile;
          } else {
            $phoneno = '';
          }

          $datasetNID = array("status" => $http_code, $type => $jsondecodeNID->$type, "suspectedMember" => "দুঃখিত, আপনার প্রদত্ত এনআইডি/স্মার্টকার্ড/জন্ম নিবন্ধন/পাসপোর্ট নম্বরটি দিয়ে ব্র্যাকের নিম্নোক্ত শাখায় সদস্য ভর্তি আছে।\n\nবিভাগ: $divisionname\nঅঞ্চল: $regionname\nএলাকা: $areaname\nশাখা: $branchname\nশাখা কোড: $branchcode\nভিও কোড: $voCode\nসদস্য নম্বর: $memberNo\nসদস্য নাম: $memberName\nপিও’র নাম: $poname\nপিও’র পিন: $po\nশাখায় যোগাযোগের নম্বর:$phoneno");
          echo json_encode($datasetNID);
        } else {
          $datasetNID = array("status" => $http_code, $type => $jsondecodeNID->$type, "suspectedMember" => "");
          echo json_encode($datasetNID);
        }
      } else {
        $data = [];
        $datasetNID = array("status" => $http_code, $type => "", "suspectedMember" => "");
        echo json_encode($datasetNID);
      }
    } else {
      if ($jsondecodeNID != null) {
  ...
  public function Savings_Transaction(Request $request)
  {
    ...
    if ($token == '7f30f4491cb4435984616d1913e88389') {
      $cdate = date('Y-m-d');
      $frmdate =date('Y-m-d', strtotime('-2 year', strtotime($cdate)));
      if ($orgno  != null and $orgmemno  != null and $branchcode != null) {
        $url = $url . "TransactionsSavings?BranchCode=$branchcode&OrgNo=$orgno&OrgMemNo=$orgmemno&key=$key&StartDate=$frmdate&EndDate=$cdate&ProjectCode=$projectcode";
      } else if ($orgmemno  != null and $branchcode != null) {
        $url = $url . "TransactionsSavings?BranchCode=$branchcode&OrgMemNo=$orgmemno&key=$key&StartDate=$frmdate&EndDate=$cdate&ProjectCode=$projectcode";
      } else {
        $result = array("status" => "E", "message" => "Please choose MemberId or Orgmemno and OrgNo!");
        return json_encode($result);
      }
      // dd($url);
      Log::info("Savings" . $url);
  ...
  public function GET_ABM_POList($branchCode, $projectcode)
  {
    $polist = DB::Table($this->db . '.polist')->where('branchcode', $branchCode)->where('projectcode', $projectcode)->where('abm', '!=', '')->get();
    return $polist;
  }
  public function Individual_GetPO($branchCode, $projectcode, $assignedpo)
  {
    $polist = DB::Table($this->db . '.polist')->where('branchcode', $branchCode)->where('projectcode', $projectcode)->where('cono', $assignedpo)->get();
    return $polist;
  }
  public function GET_POList($branchCode, $projectcode)
  {
    $polist = DB::Table($this->db . '.polist')->where('branchcode', $branchCode)->where('projectcode', $projectcode)->get();
    return $polist;
  }
