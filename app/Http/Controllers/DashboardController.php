...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (session('role_designation') == 'AM') {
            ...
            $profilepending_admission = Admission::select(DB::raw("COUNT(*) as count"))->where('projectcode', session('projectcode'))->where('Flag',2)->whereDate('created_at', '>=', $from_date)->whereDate('created_at', '<=', $today)
                ->whereIn('branchcode', $branchcode)->where('reciverrole', '!=', '0')->first();
            // dd($pending_admission);
            ...
            // dd($pending_loan);
            // status wise count**********************
            $all_pending_loan = Loans::select(DB::raw("COUNT(*) as count"))->where('status', '1')->where('projectcode', session('projectcode'))->where(DB::raw("cast(time as date)"), '>=', $from_date)->where('reciverrole', '!=', '0')->whereIn('branchcode', $branchcode)->first();
            // ->where(DB::raw("cast(time as date)"), '<=', $today)->first();
            //$all_pending_loan = DB::select(DB::raw("SELECT count(*) as count FROM $db.loans where status='1' and projectcode= '$projectcode' and reciverrole !=0 and cast(time as date) >='$from_date' and cast(time as date) <='$today'"));
            //dd($all_pending_loan[0]->count);
           ...
           // dd($all_approve_loan);
            ...
            $pending_admission = Admission::select(DB::raw("COUNT(*) as count"))->where('projectcode', session('projectcode'))->where('Flag',1)->whereDate('created_at', '>=', $from_date)->whereDate('created_at', '<=', $today)->whereIn('branchcode', $branchcode)->where('reciverrole', '!=', '0')->first();
            $profilepending_admission = Admission::select(DB::raw("COUNT(*) as count"))->where('projectcode', session('projectcode'))->where('Flag',2)->whereDate('created_at', '>=', $from_date)->whereDate('created_at', '<=', $today)->whereIn('branchcode', $branchcode)->where('reciverrole', '!=', '0')->first();
            ...
            /*$all_pending_loan = Loans::select(DB::raw("COUNT(*) as count"))->where('reciverrole', '!=', '0')->where('status', '1')->where('projectcode', session('projectcode'))
                ...
                ->whereDate($db . '.loans.time', '<=', $today)->first();*/
            //$all_pending_loan = DB::select(DB::raw("SELECT count(*) as count FROM $db.loans where status='1' and reciverrole !=0 and cast(time as date) >='$from_date' and cast(time as date) <='$today'"));
            $all_pending_loan = Loans::select(DB::raw("COUNT(*) as count"))->where('status', '1')->where('projectcode', session('projectcode'))->where(DB::raw("cast(time as date)"), '>=', $from_date)->where('reciverrole', '!=', '0')->whereIn('branchcode', $branchcode)->first();
        ...
        if (session('projectcode') == '060') {
            //dd("Huda");
            return view('progoti/Dashboard')->with('branch', $branch)->with('value', $value)->with('search2', $search2)->with('searchfilter',$searchfilter)
                ...
                ->with('all_disburse_loan', $all_disburse_loan)->with('all_reject_loan', $all_reject_loan)->with('profilepending_admission',$profilepending_admission);
        } else {
            // dd("Not Huda");
    ...
    public function statusSession(Request $req)
    {
        ...
        //dd($all_assignedpo);
        $polistAdmission = DB::table($db . '.polist')
        ...
        // date search
            ...
            // Log::info("Huda5" . $dateForm . "/" . $dateTo . '/' . json_encode($po) . '/' . session('projectcode'));
            $counts = DB::table($db . '.loans')
                ->whereIn($db . '.loans.assignedpo', $po)
        ...
        //Log::info("Huda5" . json_encode($counts));
        $pending = 0;
        $approved = 0;
        $disbursement = 0;
        $disburse = 0;
        ...
        return response()->json([$pending, $approved, $disbursement, $disburse, $rejected, $btn_bm, $btn_am, $btn_rm, $btn_dm, $totalLoan, $totalAdmission, $disburse_amt, $showStartDate, $showEndDate]);
        //return response()->json([$pending, $approved, $disbursement, $disburse, $rejected, $btn_bm, $btn_am, $btn_rm, $btn_dm, $totalLoan, $totalAdmission, $disburse_amt, $showStartDate, $showEndDate]);
    }
    ...
    public function dashboardTable(Request $req)
    {
        ...
        Log::info("Test: CheckERP" . $checkerp.'Local '.$local.'roll '.$roll);
        ...
        if ($division != null && $region == null && $area == null && $branch_search == null && $po_search == null && $roll == null) {
            ...
            if ($checkerp != 0) {
                $datas = DB::table($db . '.loans')
                    ...
                    ->whereDate($db . '.loans.time', '<=', $dateTo)
                    ->whereIn($db . '.loans.branchcode', $division_search)
                    ->where($db . '.loans.ErpStatus', $checkerp)
                    ->where($db . '.loans.reciverrole', '!=', '0')
                    ->where($db . '.loans.projectcode', session('projectcode'))
                    ->get();
            }
        ...
        if ($area != null && $branch_search == null && $po_search == null && $roll == null) {
            ...
            if ($checkerp != 0) {
                $datas = DB::table($db . '.loans')
                    ...
                    ->where($db . '.loans.reciverrole', '!=', '0')
                    ->where($db . '.loans.projectcode', $checkerp)
                    ->get();
            }
            // dd($datas);
            return datatables($datas)->addColumn('branchcode', function ($datas) {
