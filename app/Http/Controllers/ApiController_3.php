...
{
  //private $db = 'erptestingserver'; //erp test db
    ...
    if (empty($loandata)) {
      ...
      foreach ($loandata as $data) {
        ...
        $arrayData['loan'] = array(
          ...
          "DynamicFieldValue" => $data->DynamicFieldValue,
          "time" => $time,
          "dochistory_id" => $data->dochistory_id ?? 0,
          "roleid" => $data->roleid,
          "pin" => $data->pin,
          "reciverrole" => $data->reciverrole,
  ...
  public function LoanRcaDataStore(Request $request)
  {
    ...
    if ($token == '7f30f4491cb4435984616d1913e88389') {
      if ($projectcode == '015') {
        ...
        /* business info*/
        if (
          $loan_product != '79' and $loan_product != '80' and $loan_product != '100'
        ) { // trade and agri
        ...
        if ($checkData == null) {
          ...
          if ($loan_product != '79' and $loan_product != '80' and $loan_product != '100') {
            ...
            $this->ResidanceInformation(
              $db,
              $loanid,
              $residence_name,
              $residence_fatherOrHusbandName,
              $residence_idNo,
              $residence_countryName,
              $residence_workDuration,
              $residence_visaExpDate,
              $residence_loneeRelation,
              $residence_landPhoto1,
              $residence_landPhoto2,
              $residence_landPhoto3,
              $residence_landPhoto4,
              $residence_landValidation,
              $updatedate,
              $branchcode,
              $assignedpo
            );
    ...
    try {
      ...
      if ($approverwiseloanlimitmappings) {
        foreach ($approverwiseloanlimitmappings as $row) {
          if ($row->approverName == 'AM') {
            if ($row->branchGrowthType == 'HIGH') {
              if ($row->limitFrom == '5000' and $row->limitTo == '800000') {
                if ($row->repeatLimitFrom == '5000' and $row->repeatLimitTo == '800000') {
                  $groupCate = 'GC1';
                } else {
                  $groupCate = null;
                }
          ...
          } else if ($row->approverName == 'RM') {
            if ($row->branchGrowthType == 'HIGH') {
              if ($row->limitFrom == '5000' and $row->limitTo == '1200000') {
                if ($row->repeatLimitFrom == '5000' and $row->repeatLimitTo == '1200000') {
                  $groupCate = 'GC1';
                } else {
                  $groupCate = null;
                }
          ...
          } else if ($row->approverName == 'DM') {
            if ($row->branchGrowthType == 'HIGH') {
              if ($row->limitFrom == '1000001' and $row->limitTo == '2000000') {
                if ($row->repeatLimitFrom == '1000001' and $row->repeatLimitTo == '2000000') {
                  $groupCate = 'GC3';
                } else {
                  $groupCate = null;
                }
            ...
            } else if ($row->branchGrowthType == 'HIGH') {
              if ($row->limitFrom == '1200001' and $row->limitTo == '2500000') {
                if ($row->repeatLimitFrom == '1200001' and $row->repeatLimitTo == '2500000') {
                  $groupCate = 'GC1';
                } else {
                  $groupCate = null;
                }
            ...
            } else if ($row->branchGrowthType == 'MEDIUM') {
              if ($row->limitFrom == '1000001' and $row->limitTo == '2000000') {
                if ($row->repeatLimitFrom == '1000001' and $row->repeatLimitTo == '2000000') {
                  $groupCate = 'GC1';
                } else {
          ...
          DB::Table($db . '.celing_configs')->insert(['projectcode' => $row->projectCode, 'approver' => $row->approverName, 'growth_rate' => $row->branchGrowthType, 'limit_form' => $row->limitFrom, 'limit_to' => $row->limitTo, 'repeat_limit_form' => $row->repeatLimitFrom, 'repeat_limit_to' => $row->repeatLimitTo, 'group' => $groupCate]);
        }
      }
  ...
  public function Guarentors_Insert($db, $loanid, $grantor_name1, $grantor_fatherOrHusbandName1, $grantor_nidNo1, $grantor_dob1, $grantor_occupation1, $grantor_presentAddress1, $grantor_permanentAddress1, $grantor_mobileNo1, $grantor_monthlyIncome1, $grantor_monthlyExpense1, $grantor_photo1, $grantor_nidFront1, $grantor_nidBack1, $grantor_name2, $grantor_fatherOrHusbandName2, $grantor_nidNo2, $grantor_dob2, $grantor_occupation2, $grantor_presentAddress2, $grantor_permanentAddress2, $grantor_mobileNo2, $grantor_monthlyIncome2, $grantor_monthlyExpense2, $grantor_photo2, $grantor_nidFront2, $grantor_nidBack2, $updatedate, $branchcode, $assignedpo)
  {
    ...
    if ($check->isEmpty()) {
      $guarantor1 = DB::Table($db . '.guarantor_details1')->insert(['loan_id' => $loanid, 'name' => $grantor_name1, 'father_husband_name' => $grantor_fatherOrHusbandName1, 'nid_no' => $grantor_nidNo1, 'dob' => $grantor_dob1, 'occupation' => $grantor_occupation1, 'present_address' => $grantor_presentAddress1, 'parmanent_address' => $grantor_permanentAddress1, 'mobile_no' => $grantor_mobileNo1, 'monthly_income' => $grantor_monthlyIncome1, 'monthly_expenses' => $grantor_monthlyExpense1, 'guarantor_photo' => $grantor_photo1, 'guarantor_nid_front' => $grantor_nidFront1, 'guarantor_nid_back' => $grantor_nidBack1, 'name2' => $grantor_name2, 'father_husband_name2' => $grantor_fatherOrHusbandName2, 'nid_no2' => $grantor_nidNo2, 'dob2' => $grantor_dob2, 'occupation2' => $grantor_occupation2, 'present_address2' => $grantor_presentAddress2, 'parmanent_address2' => $grantor_permanentAddress2, 'mobile_no2' => $grantor_mobileNo2, 'monthly_income2' => $grantor_monthlyIncome2, 'monthly_expenses2' => $grantor_monthlyExpense2, 'guarantor_photo2' => $grantor_photo2, 'guarantor_nid_front2' => $grantor_nidFront2, 'guarantor_nid_back2' => $grantor_nidBack2, 'updated_at' => $updatedate, 'branchcode' => $branchcode, 'assignedpo' => $assignedpo]);
    } else {
      $id = $check[0]->id;
  ...
  public function ProgotiDataInsert($db, $projectcode, $data, $dataRca)
  {
  }
}
