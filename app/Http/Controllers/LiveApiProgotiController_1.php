...
{
  public function ProgotiSync($db, $apikey, $key, $token, $BranchCode, $ProjectCode, $LastSyncTime, $Appid, $Pin, $AppVersionCode, $AppversionName, $CurrentTimes)
  {
    $DataSetArray = array();
    ...
    $loandata = $this->Loan_Other_DataSync($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid);
  ...
  public function Loan_Other_DataSync($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid)
  {
    $getLoanData = array();
    $LoanRca  = array();
    $bank_info = array();
  ...
  public function MemberLists($db, $BranchCode, $ProjectCode, $Pin, $LastSyncTime, $Appid, $key)
  {
    ...
    if ($Appid == 'bmsmerp') {
      ...
      curl_close($ch);
      Log::info("MemberList" . $output_colsed);
      Log::info("MemberListUrl- " . $url4);
      if (empty($output_colsed)) {
        $this->CUSTMSG("Erp MemberList Data Set Empty!" . $http_code);
      }
      $decode = json_decode($output_colsed);
      $data = $decode->data;
      return $data;
