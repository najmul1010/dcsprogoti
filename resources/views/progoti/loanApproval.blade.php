    ...
    <div class="container">
      ...
      <div class="card card-custom">
        ...
        <div class="card-body">
          ...
          $co_qry = DB::table($db . '.polist')->where('cono', $data->assignedpo)->first();
          if ($co_qry) {
            $coname = $co_qry->coname;
            //return $coname;
          }
          /*$url = "http://scm.brac.net/sc/Individual_PO?branchcode=$data->branchcode&projectcode=$data->projectcode&cono=$data->assignedpo";
          $ch = curl_init();
          ...
          <div>
            <div class="title text_align">
              ...
              if (session('locale') == 'bn') {
                $getproject = DB::table('dcs_progoti.projects')->where('projectTitle', session('project'))->get();
                  ...
                  <div class="card-body">
                    <div class="row">
                      ...
                      <div class="col-md-8">
                        ...
                        <div class="row">
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header8') : <a target="_blank" href="{{ $admissionData->FrontSideOfIdImg }}" target="blank">image</a>
                            </p>
                          </div>
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header9') : <a target="_blank" href="{{ $admissionData->SpouseNidFront }}" target="blank">image</a>
                            </p>
                          </div>
                        </div>
                        <div class="row">
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header10') : <a target="_blank" href="{{ $admissionData->BackSideOfIdimg }}" target="blank">image</a>
                            </p>
                          </div>
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header11') : <a target="_blank" href="{{ $admissionData->SpouseNidBack }}" target="blank">image</a></p>
                          </div>
                        </div>
                        @else
                        ...
                        <div class="row">
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header8') : <a target="_blank" href="#">image</a>
                            </p>
                          </div>
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header9') : <a target="_blank" href="#">image</a></p>
                          </div>
                        </div>
                        <div class="row">
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header10') : <a target="_blank" href="#">image</a>
                            </p>
                          </div>
                          <div class="col-md-6">
                            <p>@lang('loanApproval.header11') : <a target="_blank" href="#">image</a></p>
                          </div>
                        </div>
                        @endif
              ...
              <div class="col-md-4">
                <div class="card card-custom header-section">
                  <div class="card-body">
                    ...
                    <h4>@lang('loanApproval.member_no') : {{$data->orgmemno}}</h4>
                    <p>@lang('loanApproval.header20') : {{$coname}}</p>
                    <p>@lang('loanApproval.header21') : {{$data->assignedpo}}</p>
                    <p>@lang('loanApproval.header22') : {{$data->loan_id}}</p>
                    <?php
                    if($data->erp_mem_id){
                      ?>
                      <a href="#" id="savings_behaviour">@lang('loanApproval.Savings_Behaviour')</a> <br />
                      <a href="#" id="lastCloseLoan">@lang('loanApproval.header16')</a>
                      <input type="hidden" value="<?php echo $data->erp_mem_id ?>" id="lastLoanErpMemId">
                      <input type="hidden" value="<?php echo $data->branchcode ?>" id="lastLoanBranchcode">
                      <input type="hidden" value="<?php echo $data->orgno ?>" id="orgno">
                      <input type="hidden" value="<?php echo $data->orgmemno ?>" id="orgmemno">
                      <input type="hidden" value="<?php echo session('project') ?>" id="projectcode">
                      <?php
                    }
                ...
                <div class="card card-custom header-section mt-5">
                  <div class="card-body">
                    ...
                    <?php
                    $db = config('database.db');
                    $bmComment =DB::table($db.'.document_history')->select('comment')->where('doc_id',$data->id)->where('doc_type','loan')->where('projectcode',session('projectcode'))->where('roleid',1)->orderBy('id','desc')->limit(1)->get();
                    $amComment =DB::table($db.'.document_history')->select('comment')->where('doc_id',$data->id)->where('doc_type','loan')->where('projectcode',session('projectcode'))->where('roleid',2)->orderBy('id','desc')->limit(1)->get();
                    //dd($amComment);
                    if(!$amComment->isEmpty())
                    {
                      $amComments = $amComment[0]->comment;
                    }
                    else
                    {
                      $amComments = '';
                    }
                    $rmComment =DB::table($db.'.document_history')->select('comment')->where('doc_id',$data->id)->where('doc_type','loan')->where('projectcode',session('projectcode'))->where('roleid',3)->orderBy('id','desc')->limit(1)->get();
                    if(!$rmComment->isEmpty())
                    {
                      $rmComments = $rmComment[0]->comment;
                    }
                    else
                    {
                      $rmComments = '';
                    }
                    $dmComment =DB::table($db.'.document_history')->select('comment')->where('doc_id',$data->id)->where('doc_type','loan')->where('projectcode',session('projectcode'))->where('roleid',4)->orderBy('id','desc')->limit(1)->get();
                    if(!$dmComment->isEmpty())
                    {
                      $dmComments = $dmComment[0]->comment;
                    }
                    else
                    {
                      $dmComments = '';
            ...
            <div class="box-body ">
              ...
              <div class="tab-content pre-scrollable" style="overflow-x: hidden;">
                ...
                <div class="tab-pane" id="rca">
                  ...
                  // dd($data->id);
                  $getdata = DB::table($db . '.personal_asset_info')->where('loan_id', $data->loan_id)->get();

                  if ($productcode == '100') {
                    // dd("Huda" . $productcode);
                    $gurantor = DB::table($db . '.guarantor_details1')->where('loan_id', $data->loan_id)->get();
                    $co_borrower_details = DB::table($db . '.co_borrower_details')->where('loan_id', $data->loan_id)->get();
                    $borrower_office_info = DB::table($db . '.borrower_office_info')->where('loan_id', $data->loan_id)->get();
                    $bank_info = DB::table($db . '.bank_info')->where('loan_id', $data->loan_id)->get();
                    $income_info = DB::table($db . '.income_info')->where('loan_id', $data->loan_id)->get();
                    //$expenses_info = DB::table($db . '.expenses_info')->where('loan_id', $data->id)->get();
                    $nirvorota_checklist = DB::table($db . '.checklist_nirvorota')->where('loan_id', $data->loan_id)->get();
                    // dd($gurantor[0]->name);
                  ?>
                    @include('progoti/Nirvorota')
                  <?php
                  } else if ($productcode == '79') {
                    //dd("Huda");
                    $checklist = DB::table($db . '.checklist_remitance')->where('loan_id', $data->loan_id)->get();
                    $information_remittances = DB::table($db . '.information_remittances')->where('loan_id', $data->loan_id)->get();
                    $information_abroad_resident = DB::table($db . '.information_abroad_resident')->where('loan_id', $data->loan_id)->get();
                    $bank_info = DB::table($db . '.bank_info')->where('loan_id', $data->loan_id)->get();
                    $gurantor = DB::table($db . '.guarantor_details1')->where('loan_id', $data->loan_id)->get();
                    $income_info = DB::table($db . '.income_info')->where('loan_id', $data->loan_id)->get();
                    $expenses_info = DB::table($db . '.expenses_info')->where('loan_id', $data->loan_id)->get();
                    // dd($expenses_info);
                  ?>
                    @include('progoti/Remittance')
                  <?php
                  } else if ($productcode == '80') {
                    $personal_asset_info = DB::table($db . '.personal_asset_info')->where('loan_id', $data->loan_id)->get();
                    // $expenses_info = DB::table($db . '.expenses_info')->where('loan_id', $data->id)->get();
                    //$gurantor = DB::table($db . '.guarantor_details1')->where('loan_id', $data->id)->get();
                    $co_borrower_details = DB::table($db . '.co_borrower_details')->where('loan_id', $data->loan_id)->get();
                    $borrower_passport_visa_details = DB::table($db . '.borrower_passport_visa_details')->where('loan_id', $data->loan_id)->get();
                    $otherLoanInfo = DB::Table($db . '.other_loan_information')->where('loan_id', $data->loan_id)->get();
                    $bank_info = DB::table($db . '.bank_info')->where('loan_id', $data->loan_id)->get();
                  ?>
                    @include('progoti/Migration')
                  <?php
                  }
                  //dd("Test");
                    ...
                    $income_info = DB::table($db . '.income_info')->where('loan_id', $data->loan_id)->get();
                    $expenses_info = DB::table($db . '.expenses_info')->where('loan_id', $data->loan_id)->get();
                    $business_info = DB::table($db . '.business_info')->where('loan_id', $data->loan_id)->get();
                    $trade_checklist = DB::table($db . '.checklist_trade_agri')->where('loan_id', $data->loan_id)->get();
                  ?>
                    @include('progoti/tradeAgri')

                  <?php }
                  $common_checklist = DB::table($db . '.common_checklist')->where('loan_id', $data->loan_id)->get();
                  ?>
                  @else
                  <table class="table table-bordered">
                    <tr>
                    ...
                    <table class="table table-bordered">
                      ...
                      <tr>
                        ...
                        <td colspan="2">
                          ...
                          @endphp
                          {{$maritStatus->data_name ?? "Data Not Found This maritalStatusId: $admissionApi->MaritalStatusId"}}
                          @endif
                        </td>
                      </tr>
                      <tr>
                ...
                <div class="tab-pane" id="acceptiblity">
                  ...
                  if (session('projectcode') == '60') {
                    if ($productcode == '79') {
                  ?>
                      @include('progoti/checklist_remittance');
                      @include('progoti/checklist_common');
                    <?php
                    } else if ($productcode == '100') {
                    ?>
                      @include('progoti/checklist_nirvorota');
                      @include('progoti/checklist_common');
                    <?php
                    } else if ($productcode == '80') {
                    ?>
                      @include('progoti/checklist_common');
                    <?php
                    } else if ($productcode != '79' and $productcode != '80' and $productcode != '100') {

                    ?>
                      @include('progoti/checklist_tradeAgri');
                      @include('progoti/checklist_common');
                    <?php
                    }
                  } else {
...
<script>
  $(document).ready(function() {
    ...
    $('#approve').on('click', function() {
      ...
      $("#actions").append(`<input type="hidden" value="Reject" name="action">`);
